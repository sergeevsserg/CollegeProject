...
def projectile_flight():
    if bossfight:
        for i in range(len(bullets)):
            dx = boss.x - bullets[i].x + boss.height / 2
            dy = boss.y - bullets[i].y + boss.width / 2
...
def animation():
    for count in range(50):
        ...
        win.blit(location_button.picture, (location_button.x, location_button.y + count * 10))
...
def touch_kill():
    ...
    kill_list2 = []
    if bossfight:
        if player.x - player.height / 2 <= boss.x <= player.x + player.height / 2 and player.y - player.width / 2.5 <= boss.y <= player.y + player.width / 1.2:
            if boss.damage - buff[3] - debuff[0] > 0:
                player.hp -= boss.damage - buff[3] - debuff[0]
            else:
                player.hp -= 1
        for j in range(len(bullets)):
            if (bullets[j].x - bullets[j].height <= boss.x + boss.height / 2 <= bullets[j].x + bullets[j].height and
                    bullets[j].y - bullets[j].width <= boss.y + boss.width / 2 <= bullets[j].y + bullets[j].width):
                if bullets[j].damage + buff[0] > 0:
                    boss.hp -= bullets[j].damage + buff[0]
                else:
                    boss.hp -= 0.2
                    kill_list2.append(j)
            if boss.hp <= 0:
...
def boss_moving():
    ...
    dy = player.y - boss.y
    distanse = math.sqrt(dx * dx + dy * dy)
    boss.x += boss.speed * dx / distanse
    boss.y += boss.speed * dy / distanse
    if dx > 0:
        win.blit(boss.revers_model, [boss.x, boss.y])
    else:
        win.blit(boss.model, [boss.x, boss.y])
...
locations = [pygame.image.load("pictures/bg.jpg"),
             pygame.transform.scale(pygame.image.load("pictures/pesok.jpg"), (1000, 1000)),
             pygame.transform.scale(pygame.image.load("pictures/iron.jpg"), (1000, 1000))]

image_forest = pygame.image.load("pictures/bg.jpg")
image_desert = pygame.transform.scale(pygame.image.load("pictures/pesok.jpg"), (1000, 1000))
image_factory = pygame.transform.scale(pygame.image.load("pictures/iron.jpg"), (1000, 1000))

boss = Enemy("pictures/roboboss.png", 0.8, 0, 1000, 20, 500, 500, 360, 240)
...
location_button = Menu('pictures/location_button.png', 340, 600, 300, 100)
...
upgrades = [upgrade_card("Большой калибр", 1, "pictures/up_cards/big_cal.png", 'up_p', 5, 2, 0, 0, 0),
            ...
            upgrade_card("Усовершенствованный затвор", 1, "pictures/up_cards/b_shutter.png", 'up_p', -1, -2, 0, 0, 0),
            ...
            upgrade_card("Калибр бабахи", 3, "pictures/up_cards/babaha.png", 'up_p', 20, 10, 0, 0, 0),
            upgrade_card("Пушка от A10 Thunderbolt", 3, "pictures/up_cards/a10.png", 'up_p', -40, -15, 0, 0, 0)]
...
loc_text = f1.render('', 1, (255, 0, 0))
fight_text = f1.render('ВНИМАНИЕ!!! Вы разбудиили босса, у него 1000хп и большой контактный урон', 1, (255, 0, 0))
score = 0
...
while Game:
    while menu:
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEMOTION:
                x, y = event.pos
            if event.type == pygame.MOUSEBUTTONDOWN:
                if start_button.x <= x <= start_button.x + start_button.height and start_button.y <= y <= start_button.y + start_button.width:
                    menu = False
                    run = True
                    animation()
                if exit_button.x <= x <= exit_button.x + exit_button.height and exit_button.y <= y <= exit_button.y + exit_button.width:
                    menu = False
                    Game = False
                    working = False
                if location_button.x <= x <= location_button.x + location_button.height and location_button.y <= y <= location_button.y + location_button.width:
                    ...
                    bg = locations[loc]
            if loc == 0:
                loc_text = f1.render('Обычная локация без усложнений', 1, (255, 0, 0))
            if loc == 1:
                loc_text = f1.render('+10% к скорости врагов, +10% к урону по врагам', 1, (255, 0, 0))
            if loc == 2:
                loc_text = f1.render('+10% к хп врагов, +10% к урону врагов, +50% к опыту', 1, (255, 0, 0))
    ...
    while run:
        ...
        text2 = f1.render("Счёт: ", 1, (255, 0, 0))
